...
import { LoginRequestInput, RegisterRequestInput, RegisterResponseOutput } from './dto/auth.post.dto'
...
export class AuthController {
	...
	@ApiOperation({
		...
		description:
			`Revalidates the access token and returns a new one, along with a new refresh token. Uses the refresh token stored in cookies to verify the user's identity.`,
	})
	async revalidateToken(
		...
		const { accessToken, refreshToken: newRefreshToken } = await this.authService.revalidateToken(refreshToken)
