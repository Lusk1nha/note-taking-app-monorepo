...
export function MobileRenderNavigation(props: Readonly<MobileRenderNavigationProps>) {
  const { paths } = props;
      ...
      {paths.map((path) => {
        ...
        return (
          <React.Fragment key={path.path}>
            ...
            <Separator className="h-full hidden md:block last-of-type:hidden" size="vertical" />
          </React.Fragment>
        );
      })}
    </ListFadeInAnimate>
