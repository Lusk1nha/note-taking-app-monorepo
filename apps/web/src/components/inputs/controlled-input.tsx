...
export function ControlledInput<T extends FieldValues>(props: Readonly<ControlledInputProps<T>>) {
  const { name, control, label, tip, required, field, actions, ...rest } = props;
        ...
        <fieldset className="flex flex-col gap-y-075">
          {label && (
            <div className="flex items-center justify-between">
              ...
              {actions && <div className="flex items-center gap-x-200">{actions}</div>}
            </div>
          )}
