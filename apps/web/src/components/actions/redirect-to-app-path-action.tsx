...
export function RedirectToAppPathAction(props: Readonly<RedirectToAppPathActionProps>) {
  const { path, className } = props;
  const pathname = usePathname();

  const isActive = pathname === path.path || path.aliasPaths?.includes(pathname as AppPaths);

  return (
    <Link className={cn('h-9 w-full flex cursor-pointer', className)} href={path.path}>
      <div
        className={cn(
      ...
      >
        ...
        {isActive && <ChevronIcon className="text-setting-navigation-icon-text-active" />}
      </div>
    </Link>
  );
}
